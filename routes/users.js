...
router.get('/myplan', function (req, res, next) {
  ...
  if(result.detail === 'success') {
    ...
    const query="SELECT idPlan,date_format(depart_day,'%Y-%m-%d') as depart_day,date_format(arrive_day, '%Y-%m-%d') as arrive_day,title,c.country_name,country_img,is_shared FROM triplan.Plan AS p INNER JOIN triplan.Country AS c ON p.country_id=c.idCountry WHERE userinfo_id=? and has_dayplan=1"
